...
@section('title', $produk->translated_nama_produk)
...
@section('content')
    ...
    @php
        ...
        $actualReviews = \App\Models\Review::with('user')->where('produk_id', $produk->id)->get();
            ...
            <div class="col-lg-12">
                <nav aria-label="breadcrumb" class="product-breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ $produk->translated_nama_produk }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                        ...
                        <div class="main-image-container mb-3" id="mainMediaContainer">
                            @if($produk->fotoProduk->isNotEmpty())
                                <a href="{{ asset('storage/' . $produk->fotoProduk->first()->file_foto_produk) }}" data-lightbox="product-gallery" id="mainImageLink">
                                    <img src="{{ asset('storage/' . $produk->fotoProduk->first()->file_foto_produk) }}" alt="{{ $produk->translated_nama_produk }}" id="mainProductImage" class="img-fluid"
                ...
                <div class="col-lg-6">
                    ...
                    <div class="right-content">
                        ...
                        <div class="product-header">
                            <h2 class="product-title">{{ $produk->translated_nama_produk }}</h2>
                            ...
                            @if ($usaha)
                                ...
                                <div class="shop-card">
                                    ...
                                    <img
                                        ...
                                        alt="Logo {{ $usaha->translated_nama_usaha }}"
                                    ...
                                    <div class="shop-card__info">
                                        <div class="shop-card__name">{{ $usaha->translated_nama_usaha }}</div>
                        ...
                        <div class="product-details">
                            <h5>Detail</h5>
                            <p>{{ $produk->translated_deskripsi }}</p>
                            ...
                            @foreach($produk->kategoriProduk as $kategori)
                                @if(isset($teknikDescriptions[$kategori->nama_kategori_produk]))
                                    <div id="teknik-box">
                                        <p>
                                            - {{ $kategori->translated_nama_kategori_produk }} : {{ translate_text($teknikDescriptions[$kategori->nama_kategori_produk]) }}
                                        </p>
                                    </div>
                                @endif
                            @endforeach
                            ...
                            <a href="{{ route('guest-singleProduct', $relatedProduct->slug) }}">
                                <div class="thumb">
                                    <img src="{{ asset('storage/' . optional($relatedProduct->fotoProduk->first())->file_foto_produk) }}"
                                        alt="{{ $relatedProduct->translated_nama_produk }}"
                                        onerror="this.onerror=null;this.src='{{ asset('images/produk-default.jpg') }}';">
                                </div>
                                <div class="down-content">
                                    <h4>{{ $relatedProduct->translated_nama_produk }}</h4>
                                    ...
                                    @if($relShop)
                                        <span class="product-shop" title="{{ $relShop->translated_nama_usaha }}">
                                            <i class="fa-regular fa-building"></i>{{ $relShop->translated_nama_usaha }}
                                        </span>
                                    @endif
                                </div>
                            </a>
